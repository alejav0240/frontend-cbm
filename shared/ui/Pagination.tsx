...
export function Pagination({
  ...
  const esTablet = useMediaQuery("(max-width: 1023px)");
  ...
  const getPageNumbers = () => {
    const maxVisible = esMovil ? 3 : esTablet ? 5 : 10;
    const pages = [];
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 mt-6 sm:mt-8">
      <button
        ...
        className="p-1.5 sm:p-2 rounded-xl border border-gray-200 dark:border-white/5 disabled:opacity-30 disabled:cursor-not-allowed hover:bg-gray-50 dark:hover:bg-white/5 transition-all"
      >
        ...
        {pageNumbers[0] > 1 && (
          <>
            <button
              ...
              className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl text-xs sm:text-sm font-bold text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5 transition-all"
            >
              {reverseLayer(1)}
            </button>
