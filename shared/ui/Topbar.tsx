...
import {
    Search, Bell, Sun, Moon, Menu as MenuIcon, User,
    Calendar, DollarSign, AlertCircle, Music, BookOpen, Settings as SettingsIcon, LogOut as LogOutIcon, X
} from 'lucide-react';
...
export default function Topbar() {
    ...
    const { toggleSidebar, toggleMobileMenu, setCurrentPage, setSelectedPatient } = useDashboardStore();
    ...
    const searchResults = React.useMemo((): SearchResult[] => {
        ...
        return results;
    }, [globalSearchTerm, patients, sessions, resources, setCurrentPage, setSelectedPatient]);
                            ...
                            {searchResults.length > 0 ? (
                                <div className="space-y-2">
                                    {searchResults.map((result) => (
                                        ...
                                        >
                                            <div className={`w-8 h-8 rounded-lg ${result.iconColor} flex items-center justify-center`}>
                                                {result.icon}
                                            </div>
            ...
            <header className="h-20 md:h-24 bg-white/60 dark:bg-background/60 backdrop-blur-2xl border-b border-gray-200 dark:border-white/5 px-4 md:px-8 lg:px-12 flex items-center justify-between sticky top-0 z-20 transition-all duration-500">
                ...
                <div className="flex items-center gap-4 md:gap-8 flex-1">
                    ...
                    <div className="relative max-w-md w-full hidden md:block group">
                        <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#008080] transition-colors" size={20} />
                        <input
                            type="text"
                                ...
                                <>
                                    ...
                                    >
                                        <div className="p-6 border-b border-gray-100 dark:border-white/5 flex justify-between items-center">
                                            <h3 className="font-bold dark:text-white">Notificaciones</h3>
                                            {notifications.filter(n => !n.read).length > 0 && (
                                            ...
                                            {notifications.length > 0 ? (
                                                notifications.map(notif => (
                                                    <div key={notif.id} onClick={() => markNotificationAsRead(notif.id)}>
                                                        <NotificationItem
                                                            icon={
                                                                notif.type === 'appointment' ? <Calendar size={16} /> :
                                                            ...
                                                            color={
                                                                notif.type === 'appointment' ? "bg-blue-500/10 text-blue-500" :
                                                                    notif.type === 'payment' ? "bg-green-500/10 text-green-500" :
