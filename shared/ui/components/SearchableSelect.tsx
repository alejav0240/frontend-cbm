...
export function SearchableSelect<T extends string | { label: string; value: string; color?: string }>({ 
  options, 
  value, 
  onChange, 
  onSearch,
  ...
  const getLabel = (option: T) => typeof option === 'string' ? option : option.label;
  const getValue = (option: T) => typeof option === 'string' ? option : option.value;
  const getColor = (option: T) => typeof option === 'string' ? undefined : option.color;
  ...
  React.useEffect(() => {
    if (selectedOption) {
      setLastKnownLabel(getLabel(selectedOption));
    } else if (!value) {
      setLastKnownLabel('');
    }
  }, [selectedOption, value]);

  const displayLabel = selectedOption ? getLabel(selectedOption) : lastKnownLabel;
          ...
          {isOpen && (
            <>
              ...
              <motion.div
                ...
                className={`absolute z-50 w-full min-w-[200px] bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden ${openUpwards ? 'bottom-full mb-2' : 'mt-2'}`}
              >
                ...
                <div className="max-h-60 overflow-y-auto p-2 custom-scrollbar">
                  {isLoading ? (
                    <div className="px-4 py-10 text-center">
                      <div className="w-5 h-5 border-2 border-[#008080] border-t-transparent rounded-full animate-spin mx-auto mb-2"></div>
                    ...
                    filteredOptions.map((option, idx) => {
                      ...
                      const optColor = getColor(option);
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            {optColor && (
                              <div className={`w-2 h-2 rounded-full ${optColor}`} />
                            )}
