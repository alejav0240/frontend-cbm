...
export function CalendarPicker({ 
  ...
  const renderDays = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      days.push(<div key={`empty-${viewDate.getFullYear()}-${viewDate.getMonth()}-${i}`} className="h-10" />);
    }
    ...
    <div className="relative space-y-2" ref={containerRef}>
      ...
      <div className="relative">
        ...
        >
          ...
          <span className="text-sm dark:text-white font-bold" suppressHydrationWarning>
            {value.getDate()} {months[value.getMonth()]} {value.getFullYear()}
          </span>
          <ChevronDown size={16} className={`text-gray-400 ml-2 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
        </div>
            ...
            <>
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <button onClick={handlePrevMonth} className="p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-xl transition-all text-gray-400 hover:text-[#008080]">
                    <ChevronLeft size={20} />
                  </button>
                  <h3 className="font-bold dark:text-white serif">
                    {months[viewDate.getMonth()]} {viewDate.getFullYear()}
                  </h3>
                  <button onClick={handleNextMonth} className="p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-xl transition-all text-gray-400 hover:text-[#008080]">
                ...
                <div className="grid grid-cols-7 gap-1 mb-2">
                  {['D', 'L', 'M', 'M', 'J', 'V', 'S'].map((d, i) => (
                    <div key={`${d}-${i}`} className="h-10 flex items-center justify-center text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                      {d}
                    </div>
                  ))}
                </div>
