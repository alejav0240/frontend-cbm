...
const ThemeToggle = ({ mounted, isDarkMode, onToggle }: { mounted: boolean; isDarkMode: boolean; onToggle: () => void }) => (
    <button
        onClick={onToggle}
...
const DesktopNav = ({ navLinks, isLinkActive }: {
...
const DesktopActions = ({ mounted, isDarkMode, onThemeToggle, ctaHref }: {
    mounted: boolean; isDarkMode: boolean; onThemeToggle: () => void; ctaHref: string;
}) => (
    <div className="hidden lg:flex items-center gap-4">
        <Link href="/login" className="text-sm font-bold text-gray-500 hover:text-[#008080] dark:text-gray-400 dark:hover:text-white transition-colors px-4">
            Login
        </Link>
        <ThemeToggle mounted={mounted} isDarkMode={isDarkMode} onToggle={onThemeToggle} />
        <CTALink href={ctaHref}>Agendar Sesión</CTALink>
    </div>
);

const MobileControls = ({ mounted, isDarkMode, isMenuOpen, onThemeToggle, onMenuToggle }: {
    mounted: boolean; isDarkMode: boolean; isMenuOpen: boolean; onThemeToggle: () => void; onMenuToggle: () => void;
}) => (
    <div className="lg:hidden flex items-center gap-3">
        ...
        <button
            ...
            className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${
                isMenuOpen ? 'bg-[#008080] text-white' : 'bg-gray-100 dark:bg-white/5 dark:text-white'
            }`}
...
const MobileMenu = ({ isOpen, navLinks, isLinkActive, onClose, ctaHref }: {
    isOpen: boolean; navLinks: Array<{ name: string; href: string; section: string }>;
    isLinkActive: (link: any) => boolean; onClose: () => void; ctaHref: string;
}) => (
    <AnimatePresence>
        {isOpen && (
            <>
                ...
                >
                    <div className="max-w-7xl mx-auto px-6 py-8 grid gap-4">
                        ...
                        <Link href="/login" onClick={onClose} className="flex items-center justify-center py-4 text-sm font-bold text-gray-500 dark:text-gray-400">
                            Iniciar Sesión
                        </Link>
