...
export function FormularioCrearEscala({
  ...
  const {
    ...
    resolver: zodResolver(
      esquemaCrearEscala,
    ) as unknown as Resolver<EscalaFormData>,
    defaultValues: {
      name: "",
      description: "",
      type: "subscales",
  ...
  const addSubscaleToNew = () => {
    ...
    setValue("subscales", [...subscales, { ...newSubscale, id: Date.now() }]);
  ...
  const addValueToNew = () => {
    ...
    setValue("values", [...values, { ...newValue, id: Date.now() }]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Nueva Escala de Evaluación">
                ...
                {values.map((val) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="w-6 h-6 flex items-center justify-center bg-[#008080]/10 text-[#008080] rounded-full text-[10px] font-bold">
                        {val.value}
                      </span>
                      <p className="text-xs font-bold dark:text-white">
                        {val.label}
