        ...
        return menuGroups
            .map((group) => ({
                ...
                items: group.items.filter((item) => {
                    ...
                    if (i.adminOnly && usuario?.role?.name.toLowerCase() !== "administrador") return false;
                    if (!i.permission) return true;
                    return canAccess(usuario?.modules, i.permission, "view", usuario?.role?.name);
                }),
            }))
            .filter((group) => group.items.length > 0);
    }, [sesion, usuario?.modules, usuario?.role?.name, isTutor]);
                    ...
                    {item.icon}
                                        ...
                                        {!collapsed && (
                                            <div className="flex items-center justify-between flex-1 min-w-0">
                                                ...
                                                {sesion && (
                                                    <span
                                                        className="text-[10px] font-mono font-bold text-red-400 tabular-nums ml-2 shrink-0">
        ...
        <>
            ...
            >
                ...
                >
                    <Link href="/dashboard" className="flex items-center gap-3 group">
                        <div
                            className="relative w-12 h-12 shrink-0 group-hover:rotate-12 group-hover:scale-110 transition-all duration-500">
                            <Image
            ...
            <AnimatePresence>
                {menuMovilAbierto && (
                  ...
                  <span
                      className="font-bold text-xl dark:text-white tracking-tight serif whitespace-nowrap leading-none">
                    Musico<span className="text-[#008080]">terapia</span>
                  </span>
                                    <span
                                        className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.3em] mt-1">
                    Centro Integral
                  </span>
                                </div>
                            </Link>
