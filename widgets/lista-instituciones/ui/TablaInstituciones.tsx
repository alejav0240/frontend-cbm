...
import { Building2, MapPin, Users, Mail, Phone, ChevronRight } from "lucide-react";
...
export const TablaInstituciones = ({ instituciones, alSeleccionar }: TablaInstitucionesProps) => {
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {instituciones.map((inst, idx) => (
        ...
        >
          ...
          <h3 className="text-xl font-bold dark:text-white mb-2">{inst.nombre}</h3>
          ...
          <div className="flex items-center justify-between pt-6 border-t border-gray-50 dark:border-white/5">
            <div className="flex -space-x-2">
              {inst.grupos.slice(0, 3).map((g) => (
                <div key={g.id} className="w-8 h-8 rounded-full bg-gray-100 dark:bg-white/10 border-2 border-white dark:border-[#111] flex items-center justify-center text-[10px] font-bold text-gray-400">
                  {g.nombre.charAt(0)}
                </div>
              ))}
