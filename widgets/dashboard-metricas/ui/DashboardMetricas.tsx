...
import { Users, Calendar, DollarSign, TrendingDown, ArrowUpRight, ArrowDownRight } from "lucide-react";
...
const MetricaCard = ({ titulo, valor, subtitulo, icon, color, incremento, esPositivo }: MetricaCardProps) => (
  <motion.div 
    whileHover={{ y: -5 }}
  ...
  >
    <div className="flex justify-between items-start mb-6">
      ...
      {incremento !== undefined && (
        <div className={`flex items-center gap-1 text-xs font-bold ${esPositivo ? 'text-green-500' : 'text-red-500'}`}>
          {esPositivo ? <ArrowUpRight size={14} /> : <ArrowDownRight size={14} />}
          <span>{incremento}%</span>
        </div>
      )}
    </div>
    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-1">{titulo}</p>
