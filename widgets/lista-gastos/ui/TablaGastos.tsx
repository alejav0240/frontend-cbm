      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-gray-50/50 dark:bg-white/2 border-b border-gray-100 dark:border-white/5">
            <th className="px-8 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Descripción</th>
            <th className="px-8 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Categoría</th>
            ...
            >
              <td className="px-8 py-5">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-bold dark:text-white">{gasto.descripcion}</span>
                </div>
              </td>
              <td className="px-8 py-5">
              ...
              <td className="px-8 py-5 text-sm font-bold text-red-500">
                {new Intl.NumberFormat("es-ES", { style: "currency", currency: "BOB" }).format(gasto.monto)}
              </td>
              <td className="px-8 py-5 text-xs text-gray-500">
                <div className="flex items-center gap-2">
                  <Calendar size={12} />
                  <span>{new Intl.DateTimeFormat("es-ES").format(new Date(gasto.fechaGasto))}</span>
                </div>
              </td>
              <td className="px-8 py-5">
                <span className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                  gasto.estado === "PAID" ? "bg-green-100 text-green-600" : "bg-amber-100 text-amber-600"
                }`}>
