...
export const generateSessionsListPDF = async (sessions: NormalizedSession[]) => {
    ...
    autoTable(doc, {
        head: [['#', 'Paciente', 'Fecha', 'Hora', 'Terapeuta', 'Estado', 'Pago', 'Duración']],
        body: sessions.map(s => [s.sessionNum, s.patientName, s.date, s.time, s.therapist, s.statusDisplay, s.paymentDisplay, s.duration]),
        startY: 40,
        theme: 'striped',
