        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-gray-50/50 dark:bg-white/2 border-b border-gray-100 dark:border-white/5">
              <th className="px-6 md:px-8 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">
                Sesión
              </th>
              <th className="px-6 md:px-8 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">
                Cliente
          ...
          <tbody className="divide-y divide-gray-50 dark:divide-white/5">
            {sessions.map((session, idx) => (
              ...
              >
                <td className="px-6 md:px-8 py-5 text-sm font-mono font-bold text-[#008080]">
                  #{session.sessionNum}
                </td>
                ...
                <td className="px-6 md:px-8 py-5">
                  {session.recordingUrl ? (
                    <div
                      className="flex items-center justify-center w-8 h-8 rounded-full bg-[#008080]/10 text-[#008080]"
                ...
                <td className="px-6 md:px-8 py-5">
                  <span
                    className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                      session.status.toLowerCase() === "completa"
                        ? "bg-green-100 text-green-600 dark:bg-green-500/10"
                        : session.status.toLowerCase() === "confirma"
                          ...
                          : session.status.toLowerCase() === "agendada"
                            ...
                            : session.status.toLowerCase() === "reprograma"
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${
                        session.payment.toLowerCase() === "paid"
                          ? "bg-green-500"
                          : session.payment.toLowerCase() === "exempt"
                            ? "bg-purple-500"
                            : "bg-red-500"
                      }`}
                    />
