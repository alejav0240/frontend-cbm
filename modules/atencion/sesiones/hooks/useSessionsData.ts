...
export function useSessionsData() {
    ...
    const { sessions: rawSessions, isCreating, addSession, updateSession, deleteSession, refetch } =
        useSessions({ patientId: '', paymentStatus: '', sessionStatus: '', therapistId: '', sessionType: '' });

    const { patient: patientResults, isLoading: isLoadingPatients } = useSearchPatients({ search: debouncedPatientSearch });
    const { users: therapistResults, loading: isLoadingTherapists } = useSearchUsers({ search: debouncedTherapistSearch, roleName: 'TERAPEUTA' });
    ...
    const normalizedSessions: NormalizedSession[] = useMemo(() =>
        rawSessions.map(s => {
            ...
            return {
                ...
                patientName: s.patient?.fullName ?? (s.group ? `Grupo: ${s.group.name}` : 'N/A'),
                ...
                statusDisplay: status === 'completa' ? 'Completa' : status === 'confirma' ? 'Confirma' : status === 'agendada' ? 'Agendada' : status === 'reprograma' ? 'Reprograma' : status === 'cancelada' ? 'Cancelada' : 'Pendiente',
                payment: s.paymentStatus || 'pending',
                paymentDisplay: s.paymentStatusDisplay || (payment === 'paid' ? 'Pagada' : payment === 'exempt' ? 'Exenta' : 'Pendiente'),
    ...
    const filteredSessions = useMemo(() =>
        normalizedSessions.filter(s => {
            ...
            const matchesTherapist = filterTherapist === 'Todos' || s.therapist === filterTherapist;
    ...
    const handleFormSubmit = async (data: SessionFormData) => {
        try {
            await addSession({
                ...
                notes: `${data.executionDescription ?? ''}\n\n${data.notes ?? ''}`.trim(),
            });
    ...
    const handleSaveNotes = async () => {
        ...
        try {
            await updateSession({ id: modals.selectedSession.id, notes: editedNotes });
