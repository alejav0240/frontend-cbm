...
import { SesionData, filtersData } from "@/modules/atencion/sesiones/types/session";
import { GET_SESSIONS } from "@/modules/atencion/sesiones/graphql/query";
import { useLoadingStore } from "@/shared/store/loadingStore";
import { CREATE_SESSION, UPDATE_SESSION, UPDATE_SESSION_PAYMENT, DELETE_SESSION } from "@/modules/atencion/sesiones/graphql/mutation";
...
export function useSessions(filters: filtersData) {
    ...
    const [updatePaymentMutation, { loading: isUpdating }] = useMutation(UPDATE_SESSION_PAYMENT, { onCompleted: () => refetch() });
    const [deleteMutation] = useMutation(DELETE_SESSION, { onCompleted: () => refetch() });
