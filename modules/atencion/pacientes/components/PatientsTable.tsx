      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-gray-50/50 dark:bg-white/2 border-b border-gray-100 dark:border-white/5">
            ...
            <th className="px-8 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">ID / Carnet</th>
            <th className="px-8 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Edad</th>
            <th className="px-8 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Tutor</th>
        ...
        <tbody className="divide-y divide-gray-50 dark:divide-white/5">
          {patients.map((patient, idx) => (
            ...
            >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-bold dark:text-white group-hover:text-[#008080] transition-colors">{patient.name}</p>
                    ...
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{patient?.createdAt
              ...
              <td className="px-8 py-5 text-sm font-mono text-gray-500 dark:text-gray-400">{patient.idNumber || '1234567'}</td>
              <td className="px-8 py-5 text-sm text-gray-600 dark:text-gray-400 font-medium">{patient?.birthDate
                ? differenceInYears(new Date(), new Date(patient.birthDate))
                : 0} años</td>
              <td className="px-8 py-5 text-sm text-gray-600 dark:text-gray-400">{patient.tutor?.firstName || 'N/A'}
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{patient?.tutor?.celular ? `+591 ${patient.tutor.celular}` : 'N/A'}</p>
              </td>
              <td className="px-8 py-5">
                <span className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${patient.status === 'Activo' ? 'bg-green-100 text-green-600 dark:bg-green-500/10' :
                    patient.status === 'Inactivo' ? 'bg-red-100 text-red-600 dark:bg-red-500/10' :
                      patient.status === 'Alta' ? 'bg-blue-100 text-blue-600 dark:bg-blue-500/10' :
