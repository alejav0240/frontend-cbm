...
import { TrendingUp, Camera, X, Upload, AlertCircle, Loader2 } from 'lucide-react';
import Image from 'next/image';
import { patientSchema, PatientFormData } from '@/modules/atencion/pacientes/schemas/schema';
...
export default function CreatePatientForm({ onSubmit, onCancel, isAdding = false }: CreatePatientFormProps) {
  const {
    register,
    handleSubmit,
    watch,
    ...
    <form className="space-y-6" onSubmit={handleSubmit(onFormSubmit)}>
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <input type="text" readOnly value={age !== null ? `${age} años` : ''} className="w-full px-4 py-3 bg-gray-100 dark:bg-white/10 rounded-xl border-transparent outline-none text-sm dark:text-white cursor-not-allowed" />
        </div>
      </div>
      <div className="grid sm:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Carnet / ID Paciente</label>
          <input 
            type="text" 
            {...register('idCard')}
          ...
          {photoPreview ? (
            ...
            >
              <div className="flex items-center justify-center gap-2">
                <Upload size={14} className="text-gray-400 group-hover:text-[#008080] transition-colors" />
                <p className="text-[10px] font-bold text-gray-400 group-hover:text-[#008080] uppercase tracking-widest transition-colors">Subir Foto</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Tutor / Responsable Legal</label>
          <input 
            type="text" 
            {...register('tutor')}
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Carnet del Tutor</label>
          <input 
            type="text" 
            {...register('ciTutor')}
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Teléfono del Tutor</label>
          <input 
            type="tel" 
            {...register('tutorPhone')}
            className="w-full px-4 py-3 bg-gray-50 dark:bg-white/5 rounded-xl border-transparent focus:bg-white dark:focus:bg-white/10 focus:border-[#008080] outline-none transition-all text-sm dark:text-white" 
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Email del Tutor</label>
          <input 
            type="email" 
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Residencia Actual</label>
          <input 
            type="text" 
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Diagnóstico</label>
          <input 
            type="text" 
      ...
      <div className="p-6 bg-[#008080]/5 rounded-[32px] border border-[#008080]/10 space-y-6">
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          ...
          />
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Horario de Inicio</label>
            <input 
              type="time" 
