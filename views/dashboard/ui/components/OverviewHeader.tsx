...
export function OverviewHeader({ todaySessionsCount, onStartSession, nextSession, todayNotificationsCount }: OverviewHeaderProps) {
  return (
    <div className="mb-8 md:mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
      ...
      >
        ...
        <p className="text-sm md:text-base text-gray-500 dark:text-gray-400 max-w-2xl">
          Aquí tienes un resumen de tu actividad hoy. Tienes <span className="font-bold text-[#008080]">{todaySessionsCount} sesiones</span> programadas y <span className="font-bold text-red-500">{todayNotificationsCount} notificaciones</span> que requieren tu atención.
        </p>
      </motion.div>
      ...
      <div className="flex items-center gap-3">
        <div className="hidden sm:block text-right mr-4">
          ...
          <p className="text-sm font-bold dark:text-white" suppressHydrationWarning>
            {new Date().toLocaleDateString('es-BO', { weekday: 'long', day: 'numeric', month: 'short' })}
          </p>
        </div>
        <button
          onClick={() => onStartSession?.(nextSession || { id: Date.now(), patientName: 'Paciente Demo', sessionNum: 1, date: new Date().toISOString().split('T')[0], time: 'Ahora', status: 'Confirmada', payment: 'Pendiente', duration: '45 min', therapist: 'Dra. Elena Ríos' })}
