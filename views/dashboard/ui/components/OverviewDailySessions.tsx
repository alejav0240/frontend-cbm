...
export function OverviewDailySessions({ sessions, onStartSession, onViewAll }: OverviewDailySessionsProps) {
  return (
    <motion.div 
    ...
    >
      <div className="bg-white dark:bg-[#111] rounded-[32px] p-6 md:p-8 border border-gray-200 dark:border-white/5 shadow-sm h-full flex flex-col">
        <h2 className="text-base md:text-lg font-bold mb-4 md:mb-6 dark:text-white">Sesiones de Hoy</h2>
        <div className="space-y-3 md:space-y-4 flex-1 overflow-y-auto custom-scrollbar max-h-[350px] pr-2">
          {sessions.length > 0 ? (
            ...
            <div className="py-8 text-center">
              <p className="text-xs text-gray-400 italic">No hay sesiones programadas para hoy.</p>
            </div>
          )}
        </div>
        <button 
