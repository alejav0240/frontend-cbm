...
export const SesionEnProgresoPage = () => {
    ...
    const handleFinalizar = async () => {
        ...
        const guardarDatosExitoso = !errores.includes("actualización de sesión") && errores.filter(e => e !== "grabación de video").length === 0;
        ...
        actualizarPasoGuardado(
            "completar",
            errores.length === 0 ? "completed" : errores.length === 1 && errores[0] === "grabación de video" && almacenamientoVideo === "local" ? "completed" : "error"
        );
        ...
        } else if (errores.length === 1 && errores[0] === "grabación de video" && almacenamientoVideo === "local") {
            toast.success("Sesión guardada exitosamente");
            toast.warning(
                "Cloudflare R2 no estuvo disponible. Video guardado localmente",
            );
    ...
    if (finalizando) {
        return (
            <div
                className="fixed inset-0 z-[120] bg-slate-50 dark:bg-zinc-950 flex flex-col items-center justify-center p-6 md:p-12 overflow-y-auto">
                {/* Glow effect */}
                <div
                    className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-teal-500/10 rounded-full blur-[100px] pointer-events-none"/>

                <div
                    className="w-full max-w-lg bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
                    {/* Animated top border line */}
                    <div
                        className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-teal-500 via-emerald-400 to-teal-600"/>

                    <div className="mb-6 flex flex-col items-center text-center">
                        <div
                            className="relative mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-teal-500/10 text-[#008080]">
                            <Loader2 size={36} className="animate-spin"/>
                            <div
                                className="absolute inset-0 rounded-2xl border-2 border-teal-500/20 animate-ping opacity-30 pointer-events-none"/>
                        </div>
                        ...
                        <p className="mt-2 text-sm leading-relaxed text-slate-500 dark:text-slate-400">
                            Estamos procesando la información de la sesión. No cierres esta ventana hasta completar el
                            proceso.
                        </p>
                    </div>
                    ...
                    {/* WARNING BANNER */}
                    <div
                        className="mb-6 flex items-start gap-3 rounded-2xl bg-amber-500/10 border border-amber-500/20 p-4 text-amber-800 dark:text-amber-400">
                        <AlertCircle size={18} className="shrink-0 mt-0.5 text-amber-600 dark:text-amber-500"/>
                    ...
                    <div className="space-y-3">
                        {PASOS_GUARDADO.map((paso) => {
                                ...
                                >
                                    <div className="mt-0.5 shrink-0">
                                        {completed ? (
                                            ...
                                            <Loader2 size={19} className="animate-spin text-[#008080]"/>
                                        ) : (
                                            <div
                                                className="h-5 w-5 rounded-full border-2 border-slate-300 dark:border-zinc-700"/>
                                        )}
                                    </div>
                          ...
                          <span
                              className="font-mono text-[10px] font-extrabold bg-teal-500/10 dark:bg-teal-400/10 text-[#008080] dark:text-teal-400 px-2 py-0.5 rounded-full">
