...
export function CampaignFormModal({ isOpen, onClose, onSave, initialData }: CampaignFormModalProps) {
  const [formData, setFormData] = useState<Partial<MarketingCampaign>>(initialData || DEFAULT_FORM);
    ...
    >
      <div className="space-y-6">
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Nombre de la Campaña</label>
          <input
            type="text"
            value={formData.name}
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Plataforma</label>
            <SearchableSelect
              value={formData.platform || ''}
              onChange={(val) => setFormData({ ...formData, platform: val as MarketingCampaign['platform'] })}
              options={[
                { label: 'Facebook', value: 'Facebook', color: 'bg-blue-600' },
                { label: 'Instagram', value: 'Instagram', color: 'bg-pink-600' },
          ...
          <div className="space-y-2">
            ...
            <SearchableSelect
              ...
              onChange={(val) => setFormData({ ...formData, status: val as MarketingCampaign['status'] })}
              options={[
                { label: 'Activo', value: 'Activo', color: 'bg-green-500' },
                { label: 'Pausado', value: 'Pausado', color: 'bg-blue-500' },
                { label: 'Finalizado', value: 'Finalizado', color: 'bg-gray-500' }
              ]}
              clearable={false}
            />
          </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Presupuesto (Bs.)</label>
            <input
              type="number"
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Invertido (Bs.)</label>
            <input
              type="number"
