...
const STATUS_STYLE: Record<string, { label: string; bg: string; dot: string }> = {
  ACTIVE: { label: 'Activo', bg: 'bg-emerald-500/10 text-emerald-600', dot: 'bg-emerald-500' },
  DISCHARGED: { label: 'Alta', bg: 'bg-blue-500/10 text-blue-600', dot: 'bg-blue-500' },
  INACTIVE: { label: 'Inactivo', bg: 'bg-gray-500/10 text-gray-500', dot: 'bg-gray-400' },
  PENDING: { label: 'Pendiente', bg: 'bg-amber-500/10 text-amber-600', dot: 'bg-amber-500' },
};
...
export function ExpedienteCard({ expediente, onClick, index }: ExpedienteCardProps) {
  const total = expediente.totalSessions || 1;
  const sessionProgress = Math.min((expediente.completedSessions / total) * 100, 100);

  const paidAmount = (expediente.paymentSummary?.paid ?? 0) + (expediente.paymentSummary?.exempt ?? 0);
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-5">
        ...
        <span className={`inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-widest ${statusInfo.bg}`}>
      ...
      <div className="flex items-center gap-4 mb-5 text-xs text-gray-500 font-medium">
        ...
        <span className={`flex items-center gap-1.5 font-bold uppercase tracking-widest ${paymentProgress >= 100 ? 'text-green-500' : 'text-orange-500'}`}>
          <DollarSign size={14} />
      ...
      <div className="space-y-3">
        <div className="space-y-1">
          ...
          <div className="flex justify-between items-center px-0.5">
            ...
            <span className={`text-[9px] font-bold uppercase tracking-widest ${sessionProgress >= 100 ? 'text-green-500' : 'text-[#008080]'}`}>
        ...
        <div className="space-y-1">
          ...
          <div className="flex justify-between items-center px-0.5">
            ...
            <span className={`text-[9px] font-bold uppercase tracking-widest ${paymentProgress >= 100 ? 'text-green-500' : 'text-orange-500'}`}>
