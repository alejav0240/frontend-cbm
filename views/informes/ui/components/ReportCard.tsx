    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-bold dark:text-white group-hover:text-[#008080] transition-colors">{report.title}</h3>
        ...
        <div className="flex items-center gap-4">
          <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5 ${
            ...
            {report.status === 'Leído' ? <CheckCircle2 size={12} /> : <Clock size={12} />}
            {report.status}
          </div>
          <ChevronRight size={20} className="text-gray-300 group-hover:text-[#008080] group-hover:translate-x-1 transition-all" />
        </div>
      </div>
    </motion.div>
  );
