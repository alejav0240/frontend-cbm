...
export function PaymentFormModal({ 
  ...
  const calculateTotal = (count: number, price: number, discountId?: string) => {
  ...
  const totalToPay = calculateTotal(newPayment.sessionsCount, newPayment.pricePerSession, newPayment.discountId);
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Número de Sesiones</label>
            <input 
              type="number"
              value={newPayment.sessionsCount}
              onChange={(e) => setNewPayment({...newPayment, sessionsCount: parseInt(e.target.value) || 0})}
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Precio por Sesión (Bs.)</label>
            <input 
              type="number"
              value={newPayment.pricePerSession}
              onChange={(e) => setNewPayment({...newPayment, pricePerSession: parseInt(e.target.value) || 0})}
              className="w-full px-4 py-3 bg-gray-50 dark:bg-white/5 rounded-xl border-transparent focus:bg-white dark:focus:bg-white/10 focus:border-[#008080] outline-none transition-all text-sm dark:text-white"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          />
          <div className="space-y-2">
            <label className="text-xs font-bold text-[#008080] uppercase tracking-widest">Total a Pagar (Con Descuento)</label>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Monto Pagado Hoy (Bs.)</label>
            <input 
              type="number"
            ...
            {newPayment.amountPaid < totalToPay && (
              <p className="text-[10px] text-orange-500 font-bold uppercase tracking-widest">Deuda Restante: Bs. {totalToPay - newPayment.amountPaid}</p>
            )}
          </div>
          <SearchableSelect 
        ...
        <div className="flex justify-end gap-4 pt-4">
          ...
          <button type="submit" className="bg-[#008080] text-white px-8 py-3 rounded-2xl font-bold hover:bg-[#006666] transition-all shadow-lg">Registrar Pago</button>
        </div>
      </form>
    </Modal>
  );
