...
import {FormSesionPrueba, FormSesionPruebaData} from "@/features/gestion-sesion/ui/FormSesionPrueba";
...
export const AgendaPage = () => {
    ...
    const [viewMode, setViewMode] = useState<'calendar' | 'hourly' | 'therapist'>('calendar');
    const [selectedDate, setSelectedDate] = useState(new Date());
    const [showForm, setShowForm] = useState(false);
    const [formMode, setFormMode] = useState<FormMode>('create');
    const [editingSession, setEditingSession] = useState<SesionAgenda | null>(null);
    const [showTestForm, setShowTestForm] = useState(false);
    const [showDetails, setShowDetails] = useState(false);
    const [selectedSession, setSelectedSession] = useState<SesionAgenda | null>(null);
    const [activeId, setActiveId] = useState<number | null>(null);
    const [pendingSlotHour, setPendingSlotHour] = useState<number | undefined>(undefined);

    const hours = Array.from({length: 13}, (_, i) => i + 8);

    const {sesiones, cargando, refetch} = useAgendaSessions({month: selectedDate});
    ...
    const onSlotClick = useCallback((hour: number) => {
    ...
    const handleCreateSession = useCallback(async (data: DatosCita) => {
        await crearSesion({
    ...
    const handleUpdateSession = useCallback(async (data: DatosCita) => {
    ...
    const handleStartSession = useCallback((session: SesionAgenda) => {
        setSesionActiva({
            id: session.id,
    ...
    const handleTestSubmit = useCallback(async (data: FormSesionPruebaData) => {
        try {
            await crearSesion({
                patientId: "",
    ...
    const formTitle = formMode === 'edit' ? 'Editar Sesión' : formMode === 'reschedule' ? 'Reagendar Sesión' : 'Nueva Cita';
    ...
    return (
        <div className="space-y-8">
            ...
            />
            <div className={`flex flex-col lg:flex-row gap-8 ${viewMode === 'calendar' ? 'block' : ''}`}>
                {viewMode !== 'calendar' && (
                    <AgendaSidebar
                        selectedDate={selectedDate}
                        setSelectedDate={setSelectedDate}
            ...
            >
                <FormNuevaCita
                    mode={formMode}
                    defaultDate={formMode === 'reschedule' ? undefined : selectedDate.toISOString().split("T")[0]}
                    defaultHour={formMode === 'reschedule' ? undefined : pendingSlotHour}
                    editingSession={formMode !== 'create' ? editingSession || undefined : undefined}
                    onClose={() => {
                        setShowForm(false);
                        setEditingSession(null);
                        setPendingSlotHour(undefined);
                    }}
                    onSubmit={formMode === 'create' ? handleCreateSession : handleUpdateSession}
                />
            </Modal>
            <FormSesionPrueba isOpen={showTestForm} onClose={() => setShowTestForm(false)}
                              onSubmit={handleTestSubmit} therapists={opcionesTerapeutas}
                              onSearchTherapist={() => {}} isLoadingTherapists={cargandoT}/>
            <Modal
                isOpen={showDetails}
