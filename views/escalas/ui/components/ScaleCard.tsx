...
export function ScaleCard({scale, index, isExpanded, onToggleExpand, onDelete}: ScaleCardProps) {
    return (
        <motion.div
        ...
        >
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
                <div className="flex-1">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 font-medium">{scale.descripcion}</p>
                    <div className="flex items-center gap-4 mt-4">
            <span
                className="text-[10px] font-bold text-[#008080] bg-[#008080]/10 px-3 py-1 rounded-full uppercase tracking-widest">
              {scale.tipoEscala.toLowerCase() === 'subscale' ? `${scale.subescalas?.length || 0} Subescalas` : `${scale.valores?.length || 0} Valores`}
            </span>
                    </div>
                </div>
                <div className="flex items-center gap-4 w-full lg:w-auto justify-end">
                    ...
                    >
                        {isExpanded ? (scale.tipoEscala.toLowerCase() === 'subscale' ? 'Ocultar Subescalas' : 'Ocultar Valores') : (scale.tipoEscala.toLowerCase() === 'subscale' ? 'Ver Subescalas' : 'Ver Valores')}
                                ...
                                {scale.subescalas?.map((sub: any) => (
                                    <div key={sub.id}
                                        ...
                                        <div className="flex justify-between items-start">
                                            <h4 className="font-bold dark:text-white">{sub.nombre}</h4>
                                            <span
                                                className="text-[10px] font-bold text-gray-400 uppercase">Máx: {sub.valorMaximo}</span>
                                        </div>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">{sub.description}</p>
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <div className="flex flex-wrap gap-4">
                                {scale.valores?.map((val: any) => (
                                    <div key={val.id}
                                        ...
                                        <span
                                            className="w-8 h-8 flex items-center justify-center bg-[#008080] text-white rounded-full text-xs font-bold">{val.valor}</span>
                                        <span className="text-sm font-bold dark:text-white">{val.etiqueta}</span>
                                    </div>
                                ))}
                            </div>
                        )}
