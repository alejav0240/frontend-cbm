        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/2">
              <th className="px-8 py-6 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                Formulario
              </th>
              <th className="px-8 py-6 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                Asignado a
          ...
          <tbody>
            {assignments.map((assign) => {
              const template = templates.find(
                (t) => t.id === assign.templateId,
              );
              const responsesCount = responses.filter(
                (r) => r.assignmentId === assign.id,
              ).length;
              return (
                <tr
                  key={assign.id}
                  className="border-b border-gray-50 dark:border-white/2 hover:bg-gray-50/50 dark:hover:bg-white/2 transition-colors"
                >
                  <td className="px-8 py-6">
                    <p className="text-sm font-bold dark:text-white">
                      {template?.name || "Desconocido"}
                    </p>
                  </td>
                  <td className="px-8 py-6">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-2 h-2 rounded-full ${assign.status === "Activo" ? "bg-green-500" : "bg-gray-400"}`}
                      />
