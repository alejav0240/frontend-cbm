...
const SessionCard: React.FC<SessionCardProps> = ({
    ...
    const getStatusColor = (status: string | null): string => {
        if (!status) return "bg-gray-500/10 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-500/20";
        switch (status.toLowerCase()) {
            case "completa":
            case "completed":
    ...
    const getResourceIcon = (type: string | null) => {
        ...
        return type.toUpperCase() === "AUDIO" ? <Music size={12}/> : <Video size={12}/>;
    };
    ...
    const isAudioRecording = (url: string | null): boolean => {
        if (!url) return false;
        return url.toLowerCase().includes(".mp4") || url.toLowerCase().includes(".mp3");
    };
    ...
    return (
        ...
        >
            ...
            <span
                className="text-[10px] font-bold text-[#008080] uppercase tracking-[0.2em] bg-[#008080]/10 px-4 py-1.5 rounded-full flex items-center gap-2">
              <Hash size={12}/>
              Sesión #{session.sessionNumber}
            </span>
                        <span
                            className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] flex items-center gap-2">
              <Calendar size={12}/>
              ...
              {getStatusIcon(session.sessionStatus)}
                ...
                <div className="flex flex-wrap gap-2 w-full lg:w-auto">
                    <button
                        onClick={() => onViewAIAnalysis(session.id || session.databaseId, "list")}
                    ...
                    <button
                        onClick={() => onViewAIAnalysis(session.id || session.databaseId, "charts")}
            ...
            {/* ===== MÉTRICAS RÁPIDAS ===== */}
            <div
                className="grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-8 mb-8 p-6 md:p-8 bg-gray-50/50 dark:bg-white/2 rounded-[32px] border border-gray-100 dark:border-white/5">
                <div className="space-y-1">
                    ...
                    <p className="text-sm md:text-base dark:text-white font-bold">
                        {session?.durationMinutes ? `${session.durationMinutes} min` : "40 min"}
                    </p>
                </div>
                <div className="space-y-1">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.15em] flex items-center gap-1.5">
                      ...
                      <span className="text-sm font-bold dark:text-white">
                        ...
                        evaluation.totalScore !== undefined
                            ...
                            : "N/A"}
                                        ...
                                        {evaluation.valueResponses && evaluation.valueResponses.length > 0 && (
                              ...
                              {response.scaleValue?.label || "Sin valor"}
                                        ...
                                        {evaluation.subscaleResponses && evaluation.subscaleResponses.length > 0 && (
