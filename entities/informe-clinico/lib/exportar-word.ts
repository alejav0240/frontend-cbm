...
export async function generarInformeClinicoWord(datos: InformeClinicoDTO[], nombreArchivo: string) {
    ...
    const labelValue = (label: string, value: string) =>
        new Paragraph({
            spacing: {after: 80},
            children: [
                new TextRun({text: `${label}: `, bold: true, size: 22, font: "Calibri", color: "666666"}),
    ...
    const title = (text: string) =>
        new Paragraph({
            ...
            children: [new TextRun({text, color: "008080", bold: true, size: 28, font: "Calibri"})],
        });
    ...
    const subTitle = (text: string) =>
        new Paragraph({
            ...
            children: [new TextRun({text, color: "008080", bold: true, size: 24, font: "Calibri"})],
        });
    ...
    children.push(
        new Paragraph({
            ...
            spacing: {after: 100},
            children: [new TextRun({text: "Informe Clínico", bold: true, size: 36, color: "008080", font: "Calibri"})],
        }),
        new Paragraph({
            alignment: AlignmentType.CENTER,
            spacing: {after: 400},
            children: [new TextRun({text: `Generado el: ${new Date().toLocaleDateString("es-ES")}`, size: 20, color: "999999", font: "Calibri"})],
        }),
    );
    ...
    children.push(new Paragraph({spacing: {after: 80}, children: [bold("Objetivos Generales:")]}));
    ...
    for (const [label, val] of perfiles) {
        children.push(new Paragraph({spacing: {after: 80}, children: [bold(`${label}:`)]}));
        children.push(bodyText(val));
    }

    children.push(new Paragraph({spacing: {after: 80}, children: [bold("Métodos a Usar:")]}));
    children.push(bodyText(p.metodosAUsar));

    if (p.notas) {
        children.push(new Paragraph({spacing: {after: 80}, children: [bold("Notas:")]}));
    ...
    if (informe.sesiones.length > 0) {
        ...
        const sesionCell = (text: string, width: number) =>
            new TableCell({
                width: {size: width, type: WidthType.PERCENTAGE},
                children: [new Paragraph({children: [new TextRun({text: text || "—", size: 20, font: "Calibri"})]})],
            });
        ...
        for (const escala of informe.escalas) {
            ...
            if (escala.puntuaciones.length > 0) {
                ...
                children.push(
                    new Table({
                        rows: [
                            new TableRow({children: [cellH("Sesión"), cellH("Puntuación")]}),
                            ...escala.puntuaciones.map(
                                (p) =>
                                    new TableRow({
                                        children: [cellD(p.sesion), cellD(p.valor !== null ? String(p.valor) : "—")],
                                    }),
                            ),
                        ],
                    }),
