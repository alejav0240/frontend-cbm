...
export const usePacientes = (filtros: PacienteFiltro) => {
  const { data, loading, error, refetch } = useQuery<any>(OBTENER_PACIENTES, {
    variables: {
      page: filtros.page || 1,
      pageSize: filtros.pageSize || 8,
      search: filtros.search || undefined,
      status: filtros.status === "Todos" ? undefined : filtros.status?.toLowerCase(),
    },
    notifyOnNetworkStatusChange: true,
  });

  const pacientes = useMemo(() => data?.patients?.results || [], [data]);
