...
export const useAsignacionesFormulario = (patientId?: string) => {
    ...
    const {asignaciones, respuestasForm} = useMemo(() => {
        ...
        const cleanAssignments = rawAssignments.filter((a): a is NonNullable<typeof a> => a !== null);
        ...
        for (const a of cleanAssignments) {
            ...
            const fechaAsignacion = a.createdAt ? new Date(a.createdAt as string).toLocaleDateString() : "N/A";
