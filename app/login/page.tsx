...
import { LoginFormData, loginSchema } from "@/modules/auth/types/auth.form.schema";
...
export default function Login() {
  ...
  const onFormSubmit = useCallback((data: LoginFormData) => {
    if (isBusy) return;
    login({
      variables: {
      ...
      <div className="min-h-screen bg-[#fdfcfb] dark:bg-background flex items-center justify-center p-4 relative overflow-hidden transition-colors duration-500">
        ...
        {isNavigating && (
            <div className="fixed inset-0 z-50 bg-white/80 dark:bg-background/80 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-[#008080] font-bold text-sm uppercase tracking-widest">Cargando panel...</p>
            </div>
        )}
        ...
        >
          <div className="bg-white dark:bg-accent rounded-[48px] p-10 lg:p-12 shadow-2xl border border-black/5 dark:border-white/5 transition-colors duration-500">
            <div className="text-center mb-10">
              ...
              <h1 className="serif text-3xl mb-2 dark:text-white">Panel Administrativo</h1>
              <p className="text-gray-400 text-sm">Ingresa tus credenciales para continuar</p>
            </div>

            <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-gray-400 ml-4">Usuario</label>
                <div className="relative">
                  ...
                  <input
                      ...
                      disabled={isBusy}
                  />
                  {errors.username && <p className="text-[10px] text-red-500 font-bold mt-1 ml-4">{errors.username.message}</p>}
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-gray-400 ml-4">Contraseña</label>
                <div className="relative">
                  ...
                  <input
                      ...
                      disabled={isBusy}
                  />
                  {errors.password && <p className="text-[10px] text-red-500 font-bold mt-1 ml-4">{errors.password.message}</p>}
                </div>
              </div>
              ...
              >
                {isBusy ? (
                    <>
                      ...
                      <span>{isNavigating ? 'Cargando panel...' : 'Iniciando sesión...'}</span>
                    </>
                ) : (
                    <>
                      Iniciar Sesión
